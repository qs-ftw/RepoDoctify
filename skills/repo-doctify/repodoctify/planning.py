...
def build_default_docset_plan(analysis: RepositoryAnalysis) -> DocsetPlan:
    ...
    reading_routes = {
        "30 分钟建立主心智模型": _filter_existing(
            documents,
            ["homepage", "code-reading-path", "stack-and-entrypoints"],
        ),
        "准备开始维护或接手": _filter_existing(
            documents,
            ["homepage", "module-map", "development-guide"],
        ),
        "开始排查问题和判断边界": _filter_existing(
            documents,
            ["module-map", "evidence-guide", "bridge-topics"],
        ),
        "准备新增功能或扩规则": _filter_existing(
            documents,
            ["code-reading-path", "module-map", "development-guide"],
        ),
    }
...
def _select_document_ids(analysis: RepositoryAnalysis) -> set[str]:
    ...
    selected = {"homepage", "code-reading-path", "module-map", "development-guide"}
